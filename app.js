...
async function readHardwareStreamChannel() {
    while (serialPort && serialPort.readable) {
        ...
        // Use a reliable custom LineBreakTransformer instead of dumping unbuffered raw stream text blocks
        const lineBufferStream = textDecoder.readable.pipeThrough(new TransformStream(new LineBreakTransformer()));
        serialReader = lineBufferStream.getReader();
        ...
        try {
            ...
            console.error("Hardware pipeline data reading stream error:", error);
        } finally {
...
// Line boundary chunk transformer logic to assemble incomplete hardware buffer fragments safely
class LineBreakTransformer {
    constructor() {
        this.containerBuffer = '';
    }
    transform(chunk, controller) {
        this.containerBuffer += chunk;
        const structuralLines = this.containerBuffer.split(/\r?\n/);
        this.containerBuffer = structuralLines.pop();
    ...
    flush(controller) {
        if (this.containerBuffer) {
            controller.enqueue(this.containerBuffer);
        }
    }
}
